<template>
    <div id="videoCall">
        <video class="remote-stream" ref="localVideo" id="localVideo" v-if="isStreaming">thier video</video>            
        <div class="controls">
            <button v-on:click="startCall" v-if="!isStreaming" title="start stream">
                <img src="../assets/call.png"/> 
            </button>
            <button v-on:click="stopStreaming" v-if="isStreaming" title="start stream">
                <img src="../assets/endcall.png"/>
            </button> 
            <button  v-on:click="screenShot" v-if="isStreaming && isscreenShot" title="selfie">
            ...
            <button v-on:click="startRecording" v-if="this.isStreaming" :isRecording= isRecording  title="start recording">
                <img src="../assets/recording.png"/> 
            </button>
            <button v-on:click="stopRecording" v-if="this.isStreaming && this.mediaRecorder" title="stop recording">
                <img src="../assets/stoprecording.png"/> 
            </button>    
            <button v-on:click="playStream" v-if="this.isStreaming && this.mediaRecorder" title="play record">
                <img src="../assets/playrecord.png"/> 
            </button>
            <button v-on:click="playStreamDelete" v-if="this.isStreaming && this.mediaRecorder" title="delete record">
                <img src="../assets/recordDelete.png"/> 
            </button>
            <button v-on:click="ShareScreen"  v-if="this.isStreaming"  title="sharescreen">
                <img src="../assets/sharescreen.png"/> 
            </button>                                                     
        </div>
        
            <video class="media_v" ref="webcamVideo" autoplay playsinline style="border-radius: 8px"  v-if="isStreaming"></video>
            <video class="media_v" ref="shareScreen" autoplay playsinline  v-if="isStreaming"></video>  
        
            <div class>
            <div class="container" id="container" v-if="isStreaming">
                <canvas ref="canvas"> </canvas>
                <button class="btn" v-on:click="screenShotDelete" v-if="isStreaming  && this.SHButton"  >
                    <img src="../assets/delete.png"/> 
                </button>  
            </div>  
        </div>
        <button  v-on:click="stopShareScreen" v-if="this.isStreaming && this.isShare" >
            <img src="../assets/delete.png"/> 
        </button>                 
        </div>    
        ...
        export default {
            ...
            methods:{
                ...
                screenShot: function(){
                    if(this.localStream){
                        this.SHButton = true                
                        this.$refs.canvas.width =  this.$refs.localVideo.videoWidth
    ...
    #videoCall .local-stream {
        ...
        z-index: 1000;
        background: #fff;
        border: solirgb(203, 204, 200)x rgba(255, 255, 255, 0.3);
        bottom: 0;
        left: 0%;
    }
    #videoCall .controls {
